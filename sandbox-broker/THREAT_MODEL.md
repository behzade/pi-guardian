...
Bubblewrap can mask only concrete paths. Linux expands existing secret-name glob matches before launch with packaged, compile-time-fixed `ripgrep` and discovers recursive `.git`/`.pi` controls and non-file glob matches with packaged, compile-time-fixed `find`. Patterns sharing a root use one process. Scanner processes have elapsed-time and path-size bounds; protected paths, matches, and followed symlink targets also have fixed limits and fail closed. Matching paths are then mounted after writable roots. Fixed hard denies separately protect SSH, cloud, auth, and control paths in the broker HOME. Nix-store directory symlinks are scan boundaries; ordinary user-directory symlinks are followed through bounded additional scans. The host user and Pi process are trusted, so a host-created post-snapshot secret is outside this boundary. A sandboxed command can create a new matching name in a writable tree, but that file contains data the command already controls. Linux does not claim dynamic path-pattern mediation.
