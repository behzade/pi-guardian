...
| Ripgrep glob expansion | OpenAI Codex commit `3b45c29062ff0e76e71c91b6753290400e7fa8da`, `codex-rs/linux-sandbox/src/bwrap.rs` | Design adapted in `src/linux.rs`. Pi uses packaged compile-time-fixed `ripgrep`, keeps its own exact regex as the final matcher, retains recursive `.git`/`.pi`, directory, special-file, and ordinary symlink handling through packaged compile-time-fixed `find`, and bounds subprocess time, output paths, matches, controls, and symlink targets. |
